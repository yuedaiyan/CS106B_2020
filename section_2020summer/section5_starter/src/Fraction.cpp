...
// purpose: the default constructor to create a fraction of 1 / 1
// arguments: none
...
// purpose: an overloaded constructor to create a custom
//          fraction that immediately gets reduced
// arguments: an int numerator and an int denominator
...
// purpose: to add another fraction with this one
...
// purpose: to multiply another fraction with this
//          one with the result being stored in this fraction
// arguments: another fraction
// return value: none
...
// purpose: To return a decimal value of our fraction
// arguments: None
// return value: the decimal value of this fraction
...
// purpose: To recursively find the greatest common divisor
...
/* purpose      : Converts to its reciprocal
 * arguments    : None
 * return value : None
 */
void Fraction::reciprocal() {
    int oldNumer = this->numer;
    int oldDenom = this->denom;
    this->numer = oldDenom;
    this->denom = oldNumer;
}

/* purpose      : divides the original Fraction by another Fraction
 * arguments    : another Fraction
 * return value : none
 */
...
// purpose: To overload the << operator for use with cout
// arguments: a reference to an outstream and the fraction we are using
...
bool operator<(const Fraction& first, const Fraction& second) {
    return ((double)first.numer / first.denom) < ((double)second.numer / second.denom);
}
...
STUDENT_TEST("reciprocal(): ") {
    Fraction res(1, 5);
    res.reciprocal();
    EXPECT_EQUAL(res.decimal(), 5);
}
